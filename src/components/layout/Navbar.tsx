  ...
  const navItems = [
    {
      ...
      label: "Home",
      icon: Home,
      description: "Overview & company highlights",
    },
    {
      path: "/services",
      label: "Services",
      icon: LayoutGrid,
      description: "Explore our tech capabilities",
    },
    {
      path: "/gallery",
      label: "Gallery",
      icon: Image,
      description: "Events, projects & team",
    },
    {
      path: "/contact",
      label: "Contact",
        ...
        <div className="flex items-center justify-between">
          ...
          >
            ...
            <div className="text-left">
              <span
                className={`text-xl sm:text-2xl font-extrabold tracking-tight transition-colors ${isScrolled ? "text-slate-900" : "text-white"} `}
              >
                Informa
                <span
                  className={`${isScrolled ? "text-[#003152]" : "text-white"} transition-colors group-hover:text-[#003152]'`}
                >
                  tech
                </span>
              </span>
              <span
                className={`block text-[10px] uppercase font-semibold tracking-widest -mt-1 transition-colors ${isScrolled ? "text-slate-500" : "text-slate-300"}`}
              >
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div
                      className={`text-sm font-bold ${isActive ? "text-white" : "text-slate-900"}`}
                    >
                      {item.label}
                    </div>
                    <div
                      className={`text-xs truncate ${isActive ? "text-[#a7d5fa]" : "text-slate-500"}`}
                    >
                      {item.description}
                    </div>
                  </div>
